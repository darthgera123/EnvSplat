...
from gaussian_renderer import render_uv_prior_relit
import sys
from scene import Scene_uv_prior_relit,GaussianModel_uv_prior_relit
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    scene = Scene_uv_prior_relit(dataset, gaussians)
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        render_pkg = render_uv_prior_relit(viewpoint_cam, gaussians, pipe, background,\
                                    scaling_modifier=1.0,envmap=envmap)
        
        image = render_pkg["render"]
        relit_diff = SH2RGB(render_pkg["relit_diffuse"].permute(2,0,1))
        ...
        loss += 0.01*Ll2
        # loss += 1e-1*color_loss
        # loss += 1e-1*l2_reg_output
        # loss += 1e-2*l1_pos
        loss.backward()
        ...
        with torch.no_grad():
            ...
            # Log and save
            training_report(tb_writer, iteration, Ll1, loss, l1_loss, iter_start.elapsed_time(iter_end), testing_iterations, scene, render_uv_prior_relit, (pipe, background))
            ...
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in enumerate(config['cameras']):
                    ...
                    if idx == 2 and config['name'] == 'test':
                        ...
                        np_img = np.hstack([relit_diffuse,diffuse])
                    ...
                    uvmap  = read_uvmap(viewpoint.uvmap).cuda()
                    relit_diff = SH2RGB(rendering["relit_diffuse"].permute(2,0,1))
                    color_loss += l1_loss(relit_diff,uvmap)
                ...
                color_loss /= len(config['cameras'])          
                print("\n[ITER {}] Evaluating {}: L1 {} PSNR {} Color {}".format(iteration, config['name'], l1_test, psnr_test,color_loss))
                if tb_writer:
