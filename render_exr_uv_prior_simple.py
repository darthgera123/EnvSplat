...
import torch
from scene import Scene, Scene_exr,Scene_uv_prior_simple
import os
from tqdm import tqdm
from os import makedirs
from gaussian_renderer import render,render_uv_prior_simple,render_uv_all_sh
import torchvision
from utils.general_utils import safe_state
from argparse import ArgumentParser
from arguments import ModelParams, PipelineParams, get_combined_args
from gaussian_renderer import GaussianModel,GaussianModel_uv_prior_simple
from imageio.v2 import imread,imwrite
import cv2
import numpy as np
import time
from utils.image_utils import psnr_np
def torch2numpy(img):
...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")): 
        ...
        rendering = render_uv_prior_simple(view, gaussians, pipeline, background,envmap=envmap)
        end_time = time.time()
        ...
        print("PSNR",psnr_np(exr2png(np_render),exr2png(np_gt)))
        ...
        # print(scale_img.min(),scale_img.max())
...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool):
    with torch.no_grad():
        ...
        # scene = Scene_uv_prior_simple(dataset, gaussians, load_iteration=iteration, shuffle=False)
        scene = Scene_uv_prior_simple(dataset, gaussians, shuffle=False,load_iteration=iteration)
