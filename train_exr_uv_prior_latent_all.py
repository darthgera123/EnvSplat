...
from gaussian_renderer import render_uv_prior_latent_all
import sys
from scene import Scene_uv_prior_latent_all,GaussianModel_uv_prior_latent_all
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    gaussians = GaussianModel_uv_prior_latent_all(dataset.sh_degree)
    ...
    scene = Scene_uv_prior_latent_all(dataset, gaussians)
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        render_pkg = render_uv_prior_latent_all(viewpoint_cam, gaussians, pipe, background,\
        ...
        with torch.no_grad():
            ...
            # Log and save
            training_report(tb_writer, iteration, Ll1, loss, l1_loss, iter_start.elapsed_time(iter_end), testing_iterations, scene, render_uv_prior_latent_all, (pipe, background))
            ...
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in enumerate(config['cameras']):
                    ...
                    if idx == 2 and config['name'] == 'test':
                        ...
                        # np_img = np.hstack([relit_diffuse,diffuse,scale_img])
