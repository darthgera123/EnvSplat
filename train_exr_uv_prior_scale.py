...
from gaussian_renderer import render_uv_prior_scale
import sys
from scene import Scene_uv_prior_scale,GaussianModel_uv_prior_scale
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    gaussians = GaussianModel_uv_prior_scale(dataset.sh_degree)
    ...
    scene = Scene_uv_prior_scale(dataset, gaussians)
    # gaussians.load_gt_scale()
    # gt_scale = gaussians.gt_scale
    ...
    scale_lambdas = exponential_decay(opt.iterations+1,5e-2,1e-6)
    # idmrf = IDMRFLoss()
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        render_pkg = render_uv_prior_scale(viewpoint_cam, gaussians, pipe, background,\
        ...
        with torch.no_grad():
            ...
            # Log and save
            training_report(tb_writer, iteration, Ll1, loss, l1_loss, iter_start.elapsed_time(iter_end), testing_iterations, scene, render_uv_prior_scale, (pipe, background))
            ...
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in enumerate(config['cameras']):
                    ...
                    if idx == 2 and config['name'] == 'test':
                        ...
                        np_img = np.hstack([diffuse,scale_img])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int, default=list(range(100,500_000,500)))
