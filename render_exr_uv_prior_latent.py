...
import torch
from scene import Scene, Scene_exr,Scene_uv_prior_latent
import os
from tqdm import tqdm
from os import makedirs
from gaussian_renderer import render,render_uv_prior_latent,render_uv_all_sh
import torchvision
from utils.general_utils import safe_state
from argparse import ArgumentParser
from arguments import ModelParams, PipelineParams, get_combined_args
from gaussian_renderer import GaussianModel,GaussianModel_uv_prior_latent
...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")): 
        ...
        rendering = render_uv_prior_latent(view, gaussians, pipeline, background,envmap=envmap)
        end_time = time.time()
        ...
        del envmap,gt,np_gt,uvmap
    print(f"The function took {times/len(views)} seconds to execute.")
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool):
    with torch.no_grad():
        gaussians = GaussianModel_uv_prior_latent(dataset.sh_degree)
        # scene = Scene_uv_prior_latent(dataset, gaussians, load_iteration=iteration, shuffle=False)
        scene = Scene_uv_prior_latent(dataset, gaussians, shuffle=False,load_iteration=iteration)
