...
from utils.loss_utils import l1_loss, ssim, l1_loss_exp,PerceptualLoss,IDMRFLoss,VGGLoss
from gaussian_renderer import render_uv_prior_simple
import sys
from scene import Scene_uv_prior_simple,GaussianModel_uv_prior_simple
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    gaussians = GaussianModel_uv_prior_simple(dataset.sh_degree)
    ...
    scene = Scene_uv_prior_simple(dataset, gaussians)
    ...
    # percep = VGGLoss(device='cuda',wt_mode=1)
    # loss_fn_vgg = lpips.LPIPS(net='vgg',lpips=False).cuda()
    # idmrf = IDMRFLoss()
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        uvmap  = read_uvmap(viewpoint_cam.uvmap).cuda()
        render_pkg = render_uv_prior_simple(viewpoint_cam, gaussians, pipe, background,\
        ...
        relit_diff = render_pkg["relit_diffuse"].permute(2,0,1)
        color_loss = l1_loss(relit_diff,uvmap)
        ...
        image_percep = 2*image_patches-1
        gt_percep = 2*gt_image_patches - 1
        # Ll2 = percep(image.unsqueeze(0),gt_image.unsqueeze(0))
        Ll2 = loss_fn_vgg.forward(image_percep,gt_percep)
        # Ll2 = idmrf(image,gt_image)
        # Ll2 = percep(image_patches,gt_image_patches)
        percep_loss = 1*Ll2.mean().squeeze()
        
        
        l2_reg_output = torch.norm(scale.reshape(-1,3), dim=1, keepdim=True).max()
        # l2_reg_output = torch.norm(torch.abs(scale.reshape(-1,3)), dim=1, keepdim=True).max()
        ...
        with torch.no_grad():
            ...
            # Log and save
            # training_report(tb_writer, iteration, Ll1, loss, l1_loss, iter_start.elapsed_time(iter_end), testing_iterations, scene, render_uv_prior_simple, (pipe, background))
            training_report(tb_writer, iteration, Ll1,percep_loss, loss, l1_loss, iter_start.elapsed_time(iter_end), testing_iterations, scene, render_uv_prior_simple, (pipe, background))
...
def training_report(tb_writer, iteration, Ll1,Ll2, loss, l1_loss, elapsed, testing_iterations, scene , renderFunc, renderArgs):
    if tb_writer:
        tb_writer.add_scalar('train_loss_patches/l1_loss', Ll1.item(), iteration)
        tb_writer.add_scalar('percep_patches/l1_loss', Ll2.item(), iteration)
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in enumerate(config['cameras']):
                    ...
                    if viewpoint.uvmap is not None:
                        uvmap  = read_uvmap(viewpoint.uvmap)
                    ...
                    if idx == 2 and config['name'] == 'test':
                        ...
                        uvmap = uvmap.permute(1,2,0).numpy()
                        uvmap = (uvmap*255).astype('uint8')
                        np_img = np.hstack([relit_diffuse,uvmap,scale_img])
