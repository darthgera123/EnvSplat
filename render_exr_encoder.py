...
import torch
from scene import Scene, Scene_exr,Scene_exr_encoder
import os
from tqdm import tqdm
from os import makedirs
from gaussian_renderer import render,render_encoder
import torchvision
from utils.general_utils import safe_state
from argparse import ArgumentParser
from arguments import ModelParams, PipelineParams, get_combined_args
from gaussian_renderer import GaussianModel,GaussianModel_exr_encoder
...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")): 
        ...
        envmap = view.envmap.cuda()
        
        rendering = render_encoder(view, gaussians, pipeline, background,envmap=envmap)["render"]
        end_time = time.time()
...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool):
    with torch.no_grad():
        gaussians = GaussianModel_exr_encoder(dataset.sh_degree)
        scene = Scene_exr_encoder(dataset, gaussians, load_iteration=iteration, shuffle=False)
